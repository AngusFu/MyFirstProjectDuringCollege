...
<body>
	...
	if(in_array(strtolower(texttype($_FILES['a_file']['name'])),$filetype)){ 
		if($_FILES['a_file']['size']<="900000000") { 
			...
			?>
			<?php	 
				$sql="insert into exchange(username,title,content,time,file,contact,type) values('$_SESSION[username]','{$book}','{$description}',now(),'{$name}','{$contact}','2')";
	...
	if(in_array(strtolower(texttype($_FILES['a_file1']['name'])),$filetype)){ 
		if($_FILES['a_file']['size']<="900000000") { 
			...
			?>
			<?php	 
				$sql="insert into exchange(username,title,content,time,file,contact,type) values('$_SESSION[username]','{$book1}','{$description1}',now(),'{$name}','{$contact1}','2')";
				$result=mysql_query($sql);//数据库查询的结果存入到变量$result中
				$rc = mysql_affected_rows();
				} 
			} 	 
	...
	if(in_array(strtolower(texttype($_FILES['a_file2']['name'])),$filetype)){ 
		if($_FILES['a_file']['size']<="900000000") { 
			...
			?>
			<?php	 
				$sql="insert into exchange(username,title,content,time,file,contact,type) values('$_SESSION[username]','{$book2}','{$description2}',now(),'{$name}','{$contact2}','2')";
				$result=mysql_query($sql);//数据库查询的结果存入到变量$result中
				$rc = mysql_affected_rows();
